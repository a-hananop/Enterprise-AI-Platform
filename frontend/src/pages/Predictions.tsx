    ...
    <div style={{ display: 'flex', height: 'calc(100vh - 60px)', overflow: 'hidden' }}>
      ...
      <div style={{ width: 280, flexShrink: 0, display: 'flex', flexDirection: 'column', borderRight: '1px solid var(--glass-border)', background: 'rgba(12,14,26,0.6)' }}>
        <div style={{ padding: 14, borderBottom: '1px solid var(--glass-border)' }}>
          <motion.button className="btn-primary" style={{ width: '100%', justifyContent: 'center' }} onClick={() => setShowForm(true)}
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: 8 }}>
          {loading ? Array.from({length:3}).map((_,i) => <div key={i} className="skeleton" style={{ height: 72, marginBottom: 8 }} />) :
           models.length === 0 ? (
            ...
            <motion.div key={m.id} onClick={() => setSelected(m)}
        ...
        <AnimatePresence mode="wait">
          {showForm ? (
            <motion.div key="form" style={{ maxWidth: 620 }} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.25 }}>
              ...
              {/* Model type cards with framer-motion */}
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                ...
                <motion.button className="btn-primary" onClick={train} disabled={training} style={{ alignSelf: 'flex-start' }}
            ...
            <motion.div key={selected.id} style={{ maxWidth: 700 }} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.25 }}>
              ...
              {/* 3D Radar chart for feature importance */}
              {selected.feature_importance && (
                <motion.div className="card" style={{ padding: 22, marginBottom: 20 }} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}>
                      ...
                      {Object.entries(selected.feature_importance).sort(([,a]:any,[,b]:any) => b-a).slice(0,6).map(([f,imp]:any) => {
                        ...
                        return (
                          <div key={f} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                            <span style={{ fontSize: 11, color: 'var(--text-secondary)', width: 100, flexShrink: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{f}</span>
              ...
              {/* 3D training visual when pending/running */}
